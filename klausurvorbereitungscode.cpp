...
bool isItSorted(int length, int array[])
{
    ...
    {
        if (i != length - 1)
        { // ignore last number, should be fine if loop gets this far
...
bool istAmDienstagNachmittag(Block b)
{
    ...
    if (temp >= 10 && temp <= 13)
    { // interne berechnung in funktion
        return true;
    }
    else
        return false;
...
Kunde::Kunde() : kundenNummer(freieKundenNummer++)
